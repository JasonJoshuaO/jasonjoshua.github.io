document.addEventListener('DOMContentLoaded', () => {
    ...
    //Function to render the game board on the UI/Screen
    ...
    //Function to move the tile based on the arrow key input AKA Control Func input
    ...
    function checkGameOver(){
        for(let i = 0; i < size; i++){
            for (let j = 0; j < size; i++){
                if (board[i][j] === 0){
                    return; //There is an empty cell, not game over
                }
                if (j < size - 1 && board[i][j] === board [i][j + 1]){
                    return; //There are horizontally adjacent equal cell, so a move is possible
                }
                if (i < size - 1 && board [i][j] === board [i + 1][j]){
                    return; //There are vertically adjacent equal cell, so a move is possible
                }
            }
        }
